...
const KeeperContext = React.createContext<{
  ...
  setItems: React.Dispatch<React.SetStateAction<Record<string, React.ReactNode>>>
  isExclude?: (name: string) => boolean | undefined
  keep?: (name: string, children: ReactNode, cb?: () => void) => Promise<HTMLDivElement>
}>({
...
export const KeepAlive = (props: PropsWithChildren<KeepLiveItem>) => {
  ...
  const { invisibleRefs, wrapperRef, keep, items, setItems, isMax } = useContext(KeeperContext)
